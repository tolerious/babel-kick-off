...
Babel 与生俱来就是插件化的，你可以利用已经存在的插件或者你自己创建的插件来完成定制化整个转换流程。你也可以很容的创建一个
preset，让其包含你所需要的所有
的 plugin。
...
- 通过[generator-babel-plugin](https://github.com/babel/generator-babel-plugin)
  这个工具，教程请查看[这里](./write-babel-plugin-by-generator.md)

同时Babel支持source map，能够帮助你去调试生成后的代码。Babel还支持assumption选项，简单来说，assumption选项就是告诉Babel，我的代码里没有一些奇奇怪怪的写法，例如一些极端情况：

- 把类当作普通函数调用
- 动态改原型
- ...

当我们明确的告诉Babel，我们的代码中不会出现这些代码，那么Babel就不会去处理这些极端情况，那么Babel就会编译的更快，生成的代码体积就会更小。